...
phiu_sao = mc.duplicate(phiu_phuen,n='phiu_sao')[0]
lai = mc.shadingNode('granite',at=1,n='lai_phanang')
mc.connectAttr(lai+'.oc',phiu_sao+'.c')
mc.setAttr(lai+'.fillerColor',0.5,0.2,0.2,typ='double3')
...
for i in range(nx+1):
    for j in range(nz):
        if(i==0 and j==0):
            ...
            mc.select(phanang[-1]+'.f[4:5]')
            mc.polyExtrudeFacet(sy=1.76/1.8,ltz=-0.02,kft=0)
            mc.hyperShade(a=phiu_phanang)
            mc.polyProjection(rx=0,ry=90,rz=0,psu=w/2.,psv=w/2.)
            mc.delete(*[phanang[-1]+'.f[%s]'%k for k in [0,'2:3','6:9',15,17,19,21]])
        else:
...
for i in range(nx):
    for j in range(nz+1):
        if(i==0 and j==0):
            ...
            mc.hyperShade(a=phiu_sao)
            mc.polyExtrudeFacet(*['.f[%d]'%k for k in [0,2,4,5]],sy=1.8/2)
            mc.select(phanang[-1]+'.f[0]',phanang[-1]+'.f[2]')
            mc.polyExtrudeFacet(sy=1.76/1.8,ltz=-0.02,kft=0)
            mc.hyperShade(a=phiu_phanang)
            mc.polyProjection(rx=0,ry=0,rz=0,psu=w/2.,psv=w/2.)
            mc.delete(*[phanang[-1]+'.f[%s]'%k for k in ['3:5','10:13',15,17,19,21]])
        else:
...
phanang = mc.polyUnite(sao,mc.ls(phanang),ch=0,n='phanang')
mc.move(-nx/2.*w,0,-nz/2.*w)
mc.select(mc.sets(mc.listConnections(phiu_sao+'.oc')[0],q=1))
